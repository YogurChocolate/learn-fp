    ...
    "work with turtle position with State" in {
      ...
      class MovementToWriterTState extends Natural[Movement, TurtleStateWriter] {
        override def transform[A](a: Movement[A]): TurtleStateWriter[A] = a match {
          case Start(pos) => {
            val v1 = lift[Unit, TurtleState, List[String]](State.put(pos))
            val v2 = tell[TurtleState,List[String]](List(s"starting at ${pos}"))
            for {
              _ <- v1
              _ <- v2
            } yield ()
          ...
          case MoveUp(d) => {
            val v1 = lift[Unit, TurtleState, List[String]](State.modify[Position](p => p.copy(y = p.y + d)))
            val v2 = tell[TurtleState,List[String]](List(s"moving up ${d} steps"))
            for {
              _ <- v1
              _ <- v2
            } yield ()
          ...
          case MoveDown(d) => {
            val v1 = lift[Unit, TurtleState, List[String]](State.modify[Position](p => p.copy(y = p.y - d)))
            val v2 = tell[TurtleState,List[String]](List(s"moving down ${d} steps"))
            for {
              _ <- v1
              _ <- v2
            } yield ()
          ...
          case MoveLeft(d) => {
            val v1 = lift[Unit, TurtleState, List[String]](State.modify[Position](p => p.copy(x = p.x - d)))
            val v2 = tell[TurtleState,List[String]](List(s"moving left ${d} steps"))
            for {
              _ <- v1
              _ <- v2
            } yield ()
          ...
          case MoveRight(d) => {
            val v1 = lift[Unit, TurtleState, List[String]](State.modify[Position](p => p.copy(x = p.x + d)))
            val v2 = tell[TurtleState,List[String]](List(s"moving right ${d} steps"))
            for {
              _ <- v1
              _ <- v2
            } yield ()
